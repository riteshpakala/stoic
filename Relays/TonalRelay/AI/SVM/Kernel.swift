    ...
    init(parameters: KernelParameters, data: DataSet)
    {
        ...
        if (parameters.type == .RadialBasisFunction) {
            for i in 0..<data.size {
                x_square.append(dotProduct(vector1Index: i,i))
            }
        }
    }
...
//            }
        ...
        case .RadialBasisFunction:
            var diff = [Double].init(repeating: 0.0, count: x.count)
    ...
    {
        var tmp = gamma * dotProduct(vector1Index: vector1Index, vector2Index) + coef0
        var ret = 1.0
...
//        }
    ...
    {
        return exp(-gamma*(x_square[vector1Index]+x_square[vector2Index] - 2 * dotProduct(vector1Index: vector1Index, vector2Index)))
    }
    
    func sigmoidKernel(vector1Index : Int, _ vector2Index : Int) -> Double
    {
        return tanh(gamma * dotProduct(vector1Index: vector1Index, vector2Index) + coef0)
    }
...
class SVRKernel : Kernel {
    ...
    override init(parameters: KernelParameters, data: DataSet) {
        sign = [Double].init(repeating: 1.0, count: data.size)
        sign += [Double].init(repeating: -1.0, count: data.size)
