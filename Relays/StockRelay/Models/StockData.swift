...
public class StockData: NSObject, Codable, NSCoding, NSSecureCoding {
    ...
    var updatedRSI: RSI {
        if let history = historicalData?.suffix(rsi?.maxRSI ?? 20) {//DEV: RSI max historical
            return RSI(
                open: StockKitUtils.calculateRsi(
                    history.map { $0.open } + [open]),
                close: StockKitUtils.calculateRsi(
                    history.map { $0.close } + [close]),
                maxRSI: rsi?.maxRSI ?? 20)//DEV: RSI max historical
        } else {
            return rsi ?? .init(open: 0.5, close: 0.5, maxRSI: rsi?.maxRSI ?? 20)//DEV: RSI max historical
        }
    }
