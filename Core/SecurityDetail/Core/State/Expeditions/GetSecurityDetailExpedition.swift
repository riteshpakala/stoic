...
struct GetSecurityDetailExpedition: GraniteExpedition {
    ...
    func reduce(
        ...
        let foundQuote = state.security.getQuote(moc: coreDataInstance)
        if let quote = foundQuote {
            ...
            if quote.needsUpdate {
                ...
                if let modelID = connection.retrieve(\DetailDependency.detail.modelID),
                   let model = quote.models.first(where: { $0.modelID == modelID }){
                    state.model = model
                }
            }
        } else {
            updateQuote(from: state.security, connection, foundQuote)
        }
    }
