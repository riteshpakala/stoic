...
open class AssetsPhotoViewController: UIViewController {
    ...
    fileprivate lazy var doneButtonItem: UIBarButtonItem = {
        let buttonItem = UIBarButtonItem(title: "done", style: UIBarButtonItem.Style.done, target: self, action: #selector(pressedDone(button:)))
...
//                                              action: #selector(pressedDone(button:)))
        ...
        buttonItem.setTitleTextAttributes([
            NSAttributedString.Key.font: UIFont(name: LSConst.Fonts.FiraSansBold, size: 20.0)!,
            NSAttributedString.Key.foregroundColor: LSConst.Colors.aDarkSpaceGray],
        for: .disabled)
        return buttonItem
    }()
...
extension AssetsPhotoViewController {
    ...
    func setupCommon() {
        self.navigationController?.navigationBar.titleTextAttributes = [NSAttributedString.Key.font: UIFont(name: LSConst.Fonts.PlayfairBlack, size: 20)!]
		view.backgroundColor = .ap_background
    }
...
//        let videoCount = counts.videoCount
        ...
        let titleString: String = title(forAlbum: AssetsManager.shared.selectedAlbum)
