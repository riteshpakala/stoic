...
extension AssetsPhotoLayout {
    ...
    open func expectedContentHeight(forViewSize size: CGSize, isPortrait: Bool) -> CGFloat {
        ...
        let bottomHeight = cellSize.height * 2/3 + LSConst.Device.safeAreaInsets(isPortrait: isPortrait).bottom
    ...
    private func offsetRatio(collectionView: UICollectionView, offset: CGPoint, contentSize: CGSize, isPortrait: Bool) -> CGFloat {
        return (offset.y > 0 ? offset.y : 0) / ((contentSize.height + LSConst.Device.safeAreaInsets(isPortrait: isPortrait).bottom) - collectionView.bounds.height)
    }
    ...
    open func translateOffset(forChangingSize size: CGSize, currentOffset: CGPoint) -> CGPoint? {
        ...
        if currentOffset.y < 0 {
            let insetRatio = (-currentOffset.y) / LSConst.Device.safeAreaInsets(isPortrait: isPortraitCurrent).top
            let insetDiff = LSConst.Device.safeAreaInsets(isPortrait: isPortraitFuture).top * insetRatio
