...
open class AssetsAlbumViewController: UIViewController {
    ...
    lazy var cancelButtonItem: UIBarButtonItem = {
        let buttonItem = UIBarButtonItem(title: "cancel", style: UIBarButtonItem.Style.plain, target: self, action: #selector(pressedCancel(button:)))
        
        buttonItem.setTitleTextAttributes([
            NSAttributedString.Key.font: UIFont(name: LSConst.Fonts.FiraSansBold, size: 20.0)!,
            NSAttributedString.Key.foregroundColor: LSConst.Colors.offBlack],
        for: .normal)
        return buttonItem
    }()
    ...
    lazy var collectionView: UICollectionView = {
        ...
        view.contentInset = UIEdgeInsets(top: 0, left: itemSpace, bottom: defaultSpace, right: itemSpace)
...
extension AssetsAlbumViewController {
    func setupCommon() {
//        title = "✒️"
//        title = "Albums" /// String(key: "Title_Albums")
        
//        self.navigationController?.titleTextAttributes = [NSAttributedString.Key.font: UIFont(name: LSConst.Fonts.PlayfairBlack, size: 20)!]
        
        let tlabel = UILabel(frame: .zero)
        tlabel.text = "Albums"
        tlabel.textColor = LSConst.Colors.offBlack
        tlabel.adjustsFontSizeToFitWidth = true
        tlabel.font = UIFont(name: LSConst.Fonts.PlayfairBlack, size: 20)
    ...
    public func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, referenceSizeForHeaderInSection section: Int) -> CGSize {
        if collectionView.numberOfSections > 1 && AssetsManager.shared.numberOfAlbums(inSection: 1) > 0 && section == 1 {
            if collectionView.bounds.width > collectionView.bounds.height {
                return CGSize(width: collectionView.bounds.width, height: pickerConfig.assetLandscapeCellSize(forViewSize: collectionView.bounds.size).width)
            } else {
