...
public extension Swifter {
    ...
    internal func prepareUpload(data: Data, success: JSONSwifterSuccessHandler? = nil, failure: FailureHandler? = nil) {
    ...
    internal func finalizeUpload(mediaId: String, success: JSONSwifterSuccessHandler? = nil, failure: FailureHandler? = nil) {
        let path = "media/upload.json"
        let parameters = ["command": "FINALIZE", "media_id" : mediaId]
        self.postJSON(path: path, baseURL: .upload, parameters: parameters, success: success, failure: failure)
    }
    
    internal func uploadGIF(_ mediaId: String, data: Data, name: String? = nil, success: JSONSwifterSuccessHandler? = nil, failure: FailureHandler? = nil) {
        let path = "media/upload.json"
        let parameters : [String:Any] = ["command": "APPEND", "media_id": mediaId, "segment_index": "0",
                                         ...
                                         "media": data]
        self.JSONSwifterRequest(path: path, baseURL: .upload, method: .POST, parameters: parameters, success: success, failure: failure)
    }
