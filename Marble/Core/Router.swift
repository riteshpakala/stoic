    ...
    struct AppUtility {
        static func lockToPortrait(){
            if let delegate = UIApplication.shared.delegate as? AppDelegate {
                delegate.lastRotation = .portrait
                UIDevice.current.setValue(UIInterfaceOrientation.portrait.rawValue, forKey: "orientation")
                delegate.lockRotationToLast.toggle()
            }
        }
        
        static func toggleLockToLast(){
            if let delegate = UIApplication.shared.delegate as? AppDelegate {
                delegate.lockRotationToLast.toggle()
            }
        }
        
        static func isLockedToLast() -> Bool{
            if let delegate = UIApplication.shared.delegate as? AppDelegate {
                return delegate.lockRotationToLast
            }
